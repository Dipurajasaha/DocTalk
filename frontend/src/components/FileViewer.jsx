...
import { buildAssetDownloadUrl } from '../lib/api';
...
export default function FileViewer({ file, onClose }) {
  ...
  useEffect(() => {
    ...
    const loadPreview = async () => {
      const downloadUrl = file?.download_url || (file?.id ? buildAssetDownloadUrl(file.id) : '');
      if (!downloadUrl) {
  ...
  const mimeType = String(file?.mime_type || file?.mimeType || file?.file_type || file?.fileType || '').toLowerCase();
  const displayName = file?.file_name || file?.fileName || file?.original_name || file?.name || 'Document';
  const isImage = mimeType.startsWith('image/') || /\.(png|jpg|jpeg|gif|webp)$/i.test(displayName);
  const isPdf = mimeType === 'application/pdf' || /\.pdf($|\?)/i.test(displayName);
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()} style={{ width: '90%', height: '90%', background: '#fff', borderRadius: '12px', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ flex: 1, overflow: 'auto', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 12 }}>
          {isImage && objectUrl && <img src={objectUrl} alt={displayName} style={{ maxWidth: '100%', maxHeight: '100%', borderRadius: 8 }} />}
          {isPdf && objectUrl && <iframe title={displayName} src={objectUrl} style={{ width: '100%', height: '100%', border: 'none' }} />}
          ...
          {(isImage || isPdf) && loadError && (
            <div style={{ padding: 20, color: '#64748B', textAlign: 'center' }}>
              ...
              {(file?.download_url || file?.id) && (
                <a href={file.download_url || buildAssetDownloadUrl(file.id)} target="_blank" rel="noreferrer" style={{ color: '#2b6cb0' }}>Open file in new tab</a>
              )}
            </div>
          )}
          {!isImage && !isPdf && (
            <div style={{ padding: 20 }}>
              {(file?.download_url || file?.id) ? (
                <a href={file.download_url || buildAssetDownloadUrl(file.id)} target="_blank" rel="noreferrer" style={{ color: '#2b6cb0' }}>Open {displayName} in new tab</a>
              ) : (

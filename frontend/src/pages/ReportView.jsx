...
export default function ReportView() {
  ...
  if (!report) return <div style={{padding:40}}>Report not found or access denied.</div>;

  const file = report.file || report.document || { download_url: report.url, mime_type: report.mime_type, original_name: report.filename || report.original_name || report.name };

  return (
    <div style={{ padding: 24 }}>
      <h2 style={{ marginTop: 0 }}>Report</h2>
      <div style={{ background: '#fff', padding: 16, borderRadius: 12, maxWidth: 860 }}>
        <div style={{ fontWeight: 700, marginBottom: 8 }}>{report.title || report.name || file.original_name}</div>
        <div style={{ color: '#6b7280', marginBottom: 12 }}>{report.summary || report.notes || report.description || ''}</div>

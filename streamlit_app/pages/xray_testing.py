...
def run():
    ...
    with st.expander("Consultation helper (list / create)", expanded=False):
        if not token:
            ...
            try:
                ...
                # If no consultations found, show debug info about authenticated user
                ...
                # Allow listing appointments to pick a valid appointment id
                if st.button("List appointments"):
                    try:
                        ap_res = client.list_appointments(token)
                        if ap_res.ok:
                            appts = ap_res.json()
                            # show a quick table for debugging
                            if appts:
                ...
                # allow creating a test appointment (patient token required)
                st.markdown("**Create test appointment (patient token required)**")
                # create test doctor helper
    ...
    if uploaded and st.button("Analyze Image"):
        if not patient_id:
            st.error('Patient ID is required to upload the image')
        else:
            try:
                ...
                buf.seek(0)
                files = {"file": (uploaded.name, buf, "image/png")}
                url_upload = f"{client.base_url}/api/medical_images/upload"
                ...
                if not up.ok:
                    ...
                    if not asset_id:
                        ...
                        url = f"{client.base_url}/api/processing/analyze-xray"
                        payload = {"asset_id": asset_id, "language": "en"}
                        with st.spinner('Analyzing image (this may take a while)...'):

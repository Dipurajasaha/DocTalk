...
real hospital-issued prescription, not a generic demo template:
  ...
  - A thin double-rule "security border" around the page (common on
    official medical/legal documents).
  - A repeated, very light diagonal watermark across the page (makes the
    document harder to convincingly photocopy/screenshot and reuse).
...
see prescription_service.py / prescription_pdf_storage.py for that.
"""
...
def render_prescription_pdf(prescription: dict[str, Any], doctor: Any, patient: Any) -> bytes:
    ...
    _draw_watermark(c, doctor.hospitalName or "DOCTALK HEALTH")
    ...
    y -= 6 * mm
    y = _draw_patient_panel(c, y, patient, prescription)
    y -= 10 * mm
...
def _draw_patient_panel(c: canvas.Canvas, y: float, patient: Any, prescription: dict[str, Any]) -> float:
    box_h = 14 * mm
...
def _draw_footer(c: canvas.Canvas, doctor: Any, prescription: dict[str, Any]) -> None:
    ...
    qr_buf.seek(0)
    c.drawImage(
        ImageReader(qr_buf), 18 * mm, footer_y, width=20 * mm, height=20 * mm,
    )

...
class LabResultExtractor:
    async def extract_lab_results(
        ...
        Return ONLY a JSON object with a single key "results", which contains an array of biomarker objects. Each biomarker object must have these keys:
        ...
        If no biomarkers are found, return {"results": []}.
        """
        ...
        try:
            ...
            logger.exception(
                "LabResultExtractor LLM call failed", extra={"error": str(exc)}
            )
            return []

...
class PlanningEngine:
    ...
    def determine_goal(self):
        ...
        if self.intent.is_appointment or any(k in self.text for k in [
            ...
            self.plan.goal = "manage_appointment"
        elif any(k in self.text for k in [
            ...
            self.plan.goal = "check_doctor_availability"
        elif self.intent.is_consultation or any(k in self.text for k in [
            "previous consultation", "doctor recommend", "last consultation",
            "what did doctor say", "last visit", "follow up", "recommend",
        ]):
            self.plan.goal = "review_consultation"
        elif self.intent.is_history:
            self.plan.goal = "review_patient_history"
        elif parse_document_query(self.intent.original_text) is not None or any(k in self.text for k in [
            ...
            self.plan.goal = "review_document"
        elif any(k in self.text for k in ["memory", "remember", "recall"]):
            # Added a basic fallback for MEMORY_QUERY that was missing in keywords
            self.plan.goal = "access_memory"
        else:
            self.plan.goal = "general_chat"
            
    def determine_required_information(self):
        meta = self.plan.metadata
        
        if self.plan.goal == "manage_appointment":
    ...
    def build_execution_plan(self):
        ...
        elif self.plan.goal == "access_memory":
            tasks.append(PlannerTask.create_retrieve("MEMORY"))
    ...
    def execute(self) -> ExecutionPlan:
        ...
        print(f"[DEBUG][PLANNER] goal = {self.plan.goal}")

...
:orchestrate
powershell -NoProfile -ExecutionPolicy Bypass -Command "$ErrorActionPreference='Stop'; $root=$env:DOCTALK_ROOT; $backend=$null; $frontend=$null; $shutdown=$false; $backendReady=$false; $frontendReady=$false; $backendStatus='not reachable yet'; $frontendStatus='not reachable yet'; $nextStatus=Get-Date; $handler=[ConsoleCancelEventHandler]{ param($sender,$e) $e.Cancel=$true; $script:shutdown=$true; Write-Host ''; Write-Host '[INFO] CTRL+C received. Stopping backend and frontend...' }; [Console]::add_CancelKeyPress($handler); function StopP([System.Diagnostics.Process]$p){ if($p -and -not $p.HasExited){ try{ taskkill /PID $p.Id /T /F | Out-Null } catch{} } }; try { if($env:DOCTALK_NEED_FRONTEND_INSTALL -eq '1'){ Write-Host '[INFO] Installing frontend dependencies...'; $install=Start-Process -FilePath 'cmd.exe' -ArgumentList @('/d','/s','/c',$env:DOCTALK_FRONTEND_INSTALL_CMD) -WorkingDirectory $env:DOCTALK_FRONTEND_DIR -Wait -PassThru; if($install.ExitCode -ne 0){ throw 'Frontend install failed.' } }; $backend=Start-Process -FilePath $env:DOCTALK_BACKEND_EXE -ArgumentList @('-m','uvicorn','backend.main:app','--reload','--host','127.0.0.1','--port',$env:DOCTALK_BACKEND_PORT) -WorkingDirectory $root -PassThru -NoNewWindow; $frontendCommand=$env:DOCTALK_FRONTEND_DEV_CMD + ' -- --host 127.0.0.1 --port ' + $env:DOCTALK_FRONTEND_PORT + ' --strictPort'; $frontend=Start-Process -FilePath 'cmd.exe' -ArgumentList @('/d','/s','/c',$frontendCommand) -WorkingDirectory $env:DOCTALK_FRONTEND_DIR -PassThru -NoNewWindow; Write-Host '[INFO] [6/6] Waiting for service readiness'; $deadline=(Get-Date).AddSeconds(45); while(-not $shutdown -and (-not $backendReady -or -not $frontendReady)){ if(-not $backendReady){ try{ Invoke-WebRequest -UseBasicParsing ('http://127.0.0.1:' + $env:DOCTALK_BACKEND_PORT + '/health') -TimeoutSec 2 | Out-Null; $backendReady=$true; $backendStatus='ready'; Write-Host '[OK] Backend health endpoint is reachable' }catch{} }; if(-not $frontendReady){ try{ Invoke-WebRequest -UseBasicParsing ('http://127.0.0.1:' + $env:DOCTALK_FRONTEND_PORT) -TimeoutSec 2 | Out-Null; $frontendReady=$true; $frontendStatus='ready'; Write-Host '[OK] Frontend dev server is reachable' }catch{} }; if((Get-Date) -ge $nextStatus){ $ollamaStatus=if($env:OLLAMA_READY -eq '1'){ 'reachable' } else { 'unavailable' }; Write-Host ('[STATUS] backend={0} frontend={1} postgres=healthy ollama={2}' -f $backendStatus, $frontendStatus, $ollamaStatus); $nextStatus=(Get-Date).AddSeconds(15) }; if((Get-Date) -ge $deadline -and (-not $backendReady -or -not $frontendReady)){ if(-not $backendReady){ Write-Host '[WARN] Backend health endpoint is not reachable yet.' }; if(-not $frontendReady){ Write-Host '[WARN] Frontend dev server is not reachable yet.' }; break }; Start-Sleep -Seconds 1 }; Write-Host ''; Write-Host '=============================================================='; Write-Host '  Startup Summary'; Write-Host '=============================================================='; Write-Host ('Backend:  http://127.0.0.1:{0}   [{1}]' -f $env:DOCTALK_BACKEND_PORT, $backendStatus); Write-Host ('API docs: http://127.0.0.1:{0}/docs' -f $env:DOCTALK_BACKEND_PORT); Write-Host ('Frontend: http://127.0.0.1:{0}   [{1}]' -f $env:DOCTALK_FRONTEND_PORT, $frontendStatus); Write-Host 'Database: postgres container health = healthy'; Write-Host ('Ollama:   {0}' -f $(if($env:OLLAMA_READY -eq '1'){ 'reachable' } else { 'unavailable' })); Write-Host ''; Write-Host 'Terminals started: Backend, Frontend'; Write-Host ''; Write-Host '[INFO] Development orchestrator is running. Press Ctrl+C to stop all services.'; while(-not $shutdown){ if((Get-Date) -ge $nextStatus){ $backendStatus=if($backend -and $backend.HasExited){ 'stopped' } else { 'running' }; $frontendStatus=if($frontend -and $frontend.HasExited){ 'stopped' } else { 'running' }; $ollamaStatus=if($env:OLLAMA_READY -eq '1'){ 'reachable' } else { 'unavailable' }; Write-Host ('[STATUS] backend={0} frontend={1} postgres=healthy ollama={2}' -f $backendStatus, $frontendStatus, $ollamaStatus); $nextStatus=(Get-Date).AddSeconds(15) }; if(($backend -and $backend.HasExited) -or ($frontend -and $frontend.HasExited)){ if($backend -and $backend.HasExited){ Write-Host ('[WARN] Backend exited with code ' + $backend.ExitCode + '.') }; if($frontend -and $frontend.HasExited){ Write-Host ('[WARN] Frontend exited with code ' + $frontend.ExitCode + '.') }; break }; Start-Sleep -Seconds 1 } } finally { $shutdown=$true; StopP $frontend; StopP $backend; [Console]::remove_CancelKeyPress($handler) }"
exit /b %errorlevel%
